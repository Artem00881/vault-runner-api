...
| 8a | **Data protection (GDPR Art. 28 / DPA)** | **Ready** | Processor/controller split documented; we store **only** the pseudonymous `playerId` + derived `username`/`displayName` + `locale` (no name/email/payment/KYC) — verified against `prisma/schema.prisma`. Signable **DPA** (`../commercial/dpa-template.md`), sub-processor inventory (`../commercial/sub-processors.md`), and processor record-of-processing + retention schedule + anonymize-not-delete erasure on the Art. 17(3) basis (`../commercial/data-protection-policy.md`). Erasure is operator-initiated (CLI/process today; a self-service HTTP endpoint and a scheduled retention sweep are deferred — `../api-integration-spec.md` §17). |
...
| 11 | **GLI-19 / iTech RNG certification** | **Planned (status only)** | Not certified. **GLI-19** (Interactive Gaming Systems, incl. an RNG chapter) is the primary target standard for an online RGS; **iTech Labs** RNG testing is a strong, often lower-friction alternative; **BMM** is a third option. Indicative single-game band: **~€25k–€40k initial, ~4–12 weeks**. ⚠️ **This €25k–€40k band is a SYNTHESIZED estimate, not a quoted figure** — accredited labs (GLI, iTech, BMM) **do not publish prices** and quote per project under NDA; it is the most-repeated secondary-source band for a single game/RNG, and the wider public synthesis spans roughly **~$10k–$50k initial** (with **~$5k–$25k** annual recert). The larger ~$75k–$150k / 4–6-month figures are for a **full multi-game platform** and do **not** apply to our single crash game. Get a firm number by requesting a scoped quote from the lab. (Source synthesis: `../research/b2b-market-and-compliance.md`.) |
...
| Status | Capabilities |
|---|---|
| **Ready** | Game-math spec · 1e9 simulation report · provably-fair (incl. block-salt) · API spec + Postman · **hosted operator-mode sandbox (live, `https://sandbox.vaultrun.app`)** · seamless wallet + reconciliation · responsible gambling · **data protection (GDPR Art. 28 / DPA)** · security hardening · HA/scale validation. |
...
(service levels), the data-protection set (`dpa-template.md`, `sub-processors.md`,
`data-protection-policy.md`), and the technical docs (`../api-integration-spec.md`,
